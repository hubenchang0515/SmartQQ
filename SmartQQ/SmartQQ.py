...
class SmartQQ(object):
    ...
    def group_list(self):
        if hasattr(self, '_group_list') and hasattr(self, '_group_list_update_time'):
            now = time.time()
            if now - getattr(self, '_group_list_update_time') < 1800:
                return getattr(self, '_group_list')
        ...
        setattr(self, '_group_list', rjson['result']['gnamelist'])
        setattr(self, '_group_list_update_time', time.time())
        ...
        while True:
            try:
                ...
                if (pack['type'] == "group_message"):
                    ...
                    pack['group_name'] = self.group_name(self.group_list(), pack['from_uin'])
                else:
                    pack['send_uin'] = pack['from_uin']

                pack['buddy_name'] = self.friend_name(pack['send_uin'])
                # 回答
                def reply(message):
                    uin = pack['from_uin']
